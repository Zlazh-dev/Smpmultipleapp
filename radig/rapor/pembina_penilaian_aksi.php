...
switch ($aksi) {
    case 'simpan_penilaian':
        ...
        // --- KEAMANAN: Cek Wewenang (Dipulihkan dari versi sebelumnya) ---
    ...
    case 'unduh_template':
        ...
        // --- KEAMANAN: Cek Wewenang ---
        $stmt_cek = mysqli_prepare($koneksi, "SELECT id_ekskul FROM ekstrakurikuler WHERE id_ekskul = ? AND id_pembina = ?");
        mysqli_stmt_bind_param($stmt_cek, "ii", $id_ekskul, $id_pembina);
        mysqli_stmt_execute($stmt_cek);
        if (mysqli_stmt_get_result($stmt_cek)->num_rows == 0) {
            $_SESSION['error'] = "Anda tidak memiliki wewenang untuk mengunduh template ekskul ini.";
            header("Location: pembina_penilaian_ekskul.php");
            exit();
        }
    ...
    case 'impor_excel':
        ...
        mysqli_stmt_execute($stmt_cek);
        if (mysqli_stmt_get_result($stmt_cek)->num_rows == 0) {
            $_SESSION['error'] = "Anda tidak memiliki wewenang untuk mengimpor nilai ekskul ini.";
            header("Location: pembina_penilaian_ekskul.php");
            exit();
        }
